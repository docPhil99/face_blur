...
class ImageHandler:
    ...
    def push_new_image(self, _zed_mat):
        glBindTexture(GL_TEXTURE_2D, self.image_tex)
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, _zed_mat.get_width(), _zed_mat.get_height(), GL_RGBA, GL_UNSIGNED_BYTE,
...
class GLViewer:
    ...
    def init(self, _params, _is_tracking_on, _body_format):
        ...
        glutInitWindowPosition((int)(wnd_w * 0.05),
                               (int)(wnd_h * 0.05))  # The window opens at the upper left corner of the screen
    ...
    def set_render_camera_projection(self, _params, _znear, _zfar):
        ...
        # Horizontal offset.
        self.projection.append(2 * ((_params["width"] - _params["cx"]) / _params["width"]) - 1)
        self.projection.append(0)

        self.projection.append(0)
        self.projection.append(1 / math.tan(fov_y * 0.5))  # Vertical FoV.
        # Vertical offset.
        self.projection.append(-(2 * ((_params["height"] - _params["cy"]) / _params["height"]) - 1))
        self.projection.append(0)
    ...
    def update_view(self, _bodies):  # _objs of type sl.Bodies
        self.mutex.acquire()
